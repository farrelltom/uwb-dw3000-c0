...
#define dw1000_power_valuef(COARSE,FINE) ((COARSE<<5) + (0xFFFFF & (uint16_t)(DPL_FLOAT32_INT(DPL_FLOAT32_MUL(fine, DPL_FLOAT32_INIT(2.0f))))))
...
void dw1000_phy_repeated_frames(struct _dw1000_dev_instance_t * inst, uint64_t rate);

uint16_t dw1000_phy_SHR_duration(struct uwb_phy_attributes * attrib);
uint16_t dw1000_phy_data_duration(struct uwb_phy_attributes * attrib, uint16_t nlen);
...
bool dw1000_phy_txrf_power_value(struct _dw1000_dev_instance_t * inst, uint8_t *reg, dpl_float32_t coarse, dpl_float32_t fine);

struct uwb_dev_status dw1000_phy_event_cnt_ctrl(struct _dw1000_dev_instance_t *inst, bool enable, bool reset);
struct uwb_dev_status dw1000_phy_event_cnt_read(struct _dw1000_dev_instance_t *inst, struct uwb_dev_evcnt *res);
